...
<body>
    ...
    <form class="container" method="POST" action="{{ route('log.harvester.update', ['user_id' => $user_id, 'log_id' => $log->id]) }}">
        @csrf
        @method('PUT')
        ...
        <div class="row g-3 mt-0">
            ...
            <div class="col-sm-6 col-lg-3">
                <label for="stueckzahl" class="form-label">FM Menge</label>
                <input id="stueckzahl" name="stueckzahl" class="form-control" type="number" min="0" step="0.01" inputmode="decimal"
                    value="{{ old('stueckzahl', $log->fm_amount) }}">
            </div>
        </div>

        <div class="row g-3 mt-0">
            <div class="col-sm-6 col-lg-4">
                <label for="fm_gesamt" class="form-label">FM Gesamt</label>
                <input id="fm_gesamt" name="fm_gesamt" class="form-control" type="number" min="0" step="0.01" inputmode="decimal"
                    value="{{ old('fm_gesamt', $log->fm_total) }}">
            </div>

            <div class="col-sm-6 col-lg-4">
                <label for="fm_day" class="form-label">FM Tag</label>
                <input id="fm_day" name="fm_day" class="form-control" type="number" step="0.01" inputmode="decimal"
                    value="{{ old('fm_day', $log->fm_day) }}">
            </div>
        </div>
